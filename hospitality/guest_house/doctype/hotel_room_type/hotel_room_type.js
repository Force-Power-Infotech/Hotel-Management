		...
		if (!frm.doc.__islocal) {
			frappe.call({
				...
				args: {
					...
					filters: {
						hotel_room_type_preference: frm.doc.name
					},
					limit: 1000
				},
				callback: function(r) {
					...
					prefs.forEach(pref => {
						html += `
							<a href="/app/member-details?view=list&hotel_room_type_preference=${encodeURIComponent(frm.doc.name)}&stay_preference=${encodeURIComponent(pref)}"
